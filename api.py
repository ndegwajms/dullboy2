...
def _refresh_query_params(id, type, season=None, episode=None, lang="sub"):
    params = {"id": id, "type": type, "lang": lang}
...
def _attach_refresh_metadata(result, id, type, season=None, episode=None, lang="sub"):
    if not isinstance(result, dict):
        return result

    query = urllib.parse.urlencode(_refresh_query_params(id, type, season, episode, lang))
    refresh_url = f"/refresh?{query}"
    result["refresh"] = {
        "url": refresh_url,
        "method": "GET",
        "note": "Call this endpoint when refresh_after is reached, when refresh_required is true, or when playback receives a 401/403 from an expiring source URL.",
    }

    for source in result.get("sources", []) if isinstance(result.get("sources"), list) else []:
        if isinstance(source, dict) and source.get("is_expiring"):
            source["refresh_url"] = refresh_url
...
<body>
    ...
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card">
                <h2>⛩️ Anime</h2>
                <p>Anime support is best-effort and uses episode selection plus language mode when available.</p>
        ...
        <section class="section">
            <h2>Expiring Source Refresh</h2>
            <div class="steps">
                <p>Some CDN/HLS links include signed tokens, such as <code>st=...1779154217</code>, and stop working after their embedded expiry.</p>
                <p>The API annotates those sources with <code>expires_at</code>, <code>refresh_after</code>, and <code>refresh.url</code>.</p>
                <p>Call <code>GET /refresh</code> with the same query values to force a fresh browser extraction and receive a newly signed stream URL.</p>
            </div>
            <div class="endpoint-box">GET /refresh?id=385687&type=movie</div>
        </section>
...
def refresh(
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=404, detail="Could not refresh sources")

        refreshed = _attach_refresh_metadata(result, id, type, season, episode, lang)
